...
//        }
    ...
    public void Calcular_Centroide(ArrayList<Encuesta_Respondida> muestra, int index_respuesta) {
        ...
        // TODO -> una opcion
        if (!getIsMultirespuesta()) {
            // TODO calcular moda

            this.setValor( calcular_moda_uniopcion(muestra, index_respuesta));

        } //else { //TODO Más opciones }
    ...
    public static int max_int_de_lista(int[] lista){
        ...
        for (int i = 0; i < lista.length; i++ ) {
            max = (max < lista[i])? lista[i] : max;
            max_index = ((max < lista[i])?  i : max_index);
        }
        return max_index;
    }
